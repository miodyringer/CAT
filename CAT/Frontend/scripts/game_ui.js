  ...
  players.forEach(player => {
    ...
    if (player.uuid !== localPlayerId) {
      ...
      kickButton.onclick = async () => {
        if (await window.confirm(translate(getCookie('language'), 'player_kick_confirm').replace('{player}', player.name))) {
...
async function executePlay (extraDetails = {}) {
  ...
  if (playedCard.type === 'JokerCard') {
    const jokerImitation = gameService.getJokerImitation()
    if (!jokerImitation) {
      window.alert(translate(getCookie('language'), 'joker_card_select_alert'))
      return
    }
  ...
  if (playedCard.type === 'InfernoCard') {
    ...
    const figureId = gameService.getSelectedFigureId()
    if (!figureId) {
      window.alert(translate(getCookie('language'), 'figure_not_selected_alert'))
      return
    }
...
async function executeVoteKick (playerToKickNumber) {
  ...
  try {
    ...
    window.alert(`Could not vote: ${error.message}`)
  }
}
...
async function initializeGame () {
  ...
  if (!gameId || !localPlayerId) {
    window.alert(translate(getCookie('language'), 'game_url_alert'))
  ...
  try {
    ...
    if (!gameStateFromServer) {
      window.alert(translate(getCookie('language'), 'load_data_alert'))
      return
    }
    ...
    const wsUrl = `${config.webSocketUrl}/game/ws/${gameId}/${localPlayerId}`
    ...
    socket.onmessage = (event) => {
      ...
      switch (message.event) {
        ...
        case 'game_closed':
          ...
          window.alert(`${translate(getCookie('language'), 'game_closed_alert')} ${message.reason}.`)
    ...
    if (!startGameBtn.dataset.listenerAttached) {
      startGameBtn.addEventListener('click', async () => {
        try {
          ...
          window.alert(translate(getCookie('language'), 'start_game_alert'))
          console.error(error)
        }
      })
    ...
    window.alert(translate(getCookie('language'), 'load_game_alert'))
  }
}
